...
contract TestUpdateMinters is Test {
    ...
    bool internal hasMainnetRpc;

    function setUp() public {
        string memory mainnetRpc;
        try vm.envString("ETH_RPC_MAINNET") returns (string memory rpc) {
            mainnetRpc = rpc;
        } catch {
            return;
        }
        ...
        hasMainnetRpc = true;
        uint256 mainnetFork = vm.createFork(mainnetRpc, 16585958);
        vm.selectFork(mainnetFork);
    }

    function testUpdateMinters() public {
        if (!hasMainnetRpc) return;
        ...
        ////////      someone proposes baraza dao upgrade and airdrop      ////////
