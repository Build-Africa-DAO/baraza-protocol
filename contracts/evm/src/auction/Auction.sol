...
/// Modified from:
/// - BarazaAuctionHouse.sol commit 2cbe6c7 - licensed under the BSD-3-Clause license.
...
contract Auction is IAuction, VersionedContract, UUPS, Ownable, ReentrancyGuard, Pausable, AuctionStorageV1, AuctionStorageV2 {
    ...
    /// @notice The baraza reward BPS as a percent of settled auction amount
    uint16 public immutable barazaRewardsBPS;
    ...
    function _computeTotalRewards(
        ...
        // Get global baraza recipient from manager
        address barazaRecipient = manager.barazaRewardsRecipient();

        // Calculate the total rewards percentage
        uint256 totalBPS = _founderRewardBps + referralRewardsBPS + barazaRewardsBPS;
        ...
        // Set baraza reward
        uint256 barazaAmount = (_finalBidAmount * barazaRewardsBPS) / BPS_PER_100_PERCENT;
        split.recipients[0] = barazaRecipient;
        split.amounts[0] = barazaAmount;
        split.reasons[0] = REWARDS_REASON;
        split.totalRewards += barazaAmount;

        // Set referral reward
        uint256 referralAmount = (_finalBidAmount * referralRewardsBPS) / BPS_PER_100_PERCENT;
        split.recipients[1] = _currentBidRefferal != address(0) ? _currentBidRefferal : barazaRecipient;
    ...
    function _authorizeUpgrade(address _newImpl) internal view override onlyOwner whenPaused {
        // Ensure the new implementation is registered by the Baraza DAO
