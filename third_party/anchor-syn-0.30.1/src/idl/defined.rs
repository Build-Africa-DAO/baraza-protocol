    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            #[cfg(procmacro2_semver_exempt)]
            {
                ...
                let source_path = std::env::var_os("CARGO_MANIFEST_DIR")
                    .map(std::path::PathBuf::from)
                    .unwrap_or_else(|| std::env::current_dir().expect("current directory should exist"))
                    .join("src")
                    .join("lib.rs");
                let lib_path = if source_path.exists() {
                    source_path.clone()
                } else {
                    find_path("lib.rs", &std::env::current_dir().expect("current directory should exist"))
