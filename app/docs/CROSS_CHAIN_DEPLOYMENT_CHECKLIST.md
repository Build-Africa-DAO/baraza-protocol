...
- [ ] **⚠️ HIGH RISK - Deployer EOA is temporary; ownership transfer to the correct multisig is scripted and tested**
...
- [ ] **⚠️ HIGH RISK - OP Stack sequencer liveness assumption reviewed in proposal execution logic**
...
- [ ] **⚠️ HIGH RISK - Block-dependent logic reviewed; use `ArbSys.arbBlockNumber()` if Arbitrum block number is required**
...
- [ ] **⚠️ HIGH RISK - Chainlink feed heartbeat confirmed; some Polygon feeds update less frequently**
- [ ] Gnosis Safe deployed and tested
...
- [ ] **⚠️ HIGH RISK - OP Stack sequencer dependency reviewed in proposal execution logic**
...
- [ ] RPC endpoint confirmed and fallback set
- [ ] N/A - Block explorer API key for EVM-style source verification; use Solana explorer/program metadata process instead
...
- [ ] Anchor framework version pinned
- [ ] Program derived addresses documented for all accounts
- [ ] **⚠️ HIGH RISK - CPI attack surface reviewed even though EVM reentrancy does not apply directly**
- [ ] Rent-exempt minimum balance pre-funded for all required accounts
- [ ] **⚠️ HIGH RISK - Gnosis Safe is EVM-only; Squads or native multisig replacement confirmed**
...
- [ ] RPC endpoint confirmed and fallback set
- [ ] N/A - Block explorer API key for EVM-style verification; use Stellar/Soroban verification process instead
...
- [ ] **⚠️ HIGH RISK - Stellar / Soroban is not EVM; LayerZero and Wormhole support must be explicitly verified**
- [ ] **⚠️ HIGH RISK - Chainlink does not natively support Stellar; alternative oracle strategy required**
- [ ] **⚠️ HIGH RISK - Gnosis Safe is not available on Stellar; use Stellar multisig accounts with threshold signatures**
...
- [ ] **⚠️ HIGH RISK - Cross-chain proposal execution path tested end-to-end on testnet**
...
- [ ] **⚠️ HIGH RISK - Trusted remote / peer addresses set correctly on every chain; mismatch can silently drop messages**
...
- [ ] **⚠️ HIGH RISK - Replay protection enabled with nonces, unique message IDs, or domain separators**
- [ ] Cross-chain domain separators include chain ID where applicable
- [ ] Message delivery failure fallback logic implemented
- [ ] **⚠️ HIGH RISK - Stellar bridge support explicitly confirmed with LayerZero / Wormhole before relying on it**
...
- [ ] **⚠️ HIGH RISK - Answer range validated: `require(answer > minAnswer && answer < maxAnswer)`**
...
- [ ] **⚠️ HIGH RISK - Ownership transferred to multisig, not deployer EOA**
...
- [ ] **⚠️ HIGH RISK - External audit completed and all critical / high findings remediated**
- [ ] **⚠️ HIGH RISK - Bug bounty program live before mainnet governance is active**
...
**NOT READY - audit not started; Stellar oracle and bridge support unconfirmed; Gnosis Safe replacement on Solana and Stellar not yet decided.**
