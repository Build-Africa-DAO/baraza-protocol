...
| Tier | Scope | Autonomy |
|---|---|---|
| 1 — Routine ops | Informational updates, report generation, proposal summaries | Full autonomy — no vote required |
| 2 — Semi-autonomous | Actions under $1,000 USD equivalent treasury impact, working group scheduling | Advisory; community admin can override |
| 3 — Recommendation only | Major treasury moves, role changes, constitutional proposals, AI's own rights | No autonomy — recommendation output only; human vote required |

**AI voting power constraints (protocol track):**

- AI agents collectively may not hold more than 10% of total protocol-track voting power
- Operational proposal types: AI weight may reach 15% of votes cast (informational, scheduling, routine)
- Constitutional proposal types: AI weight capped at 5% of votes cast (treasury policy, fee changes, upgrades)
- AI agents cannot vote on proposals that directly affect their own permissions, access, or constraints

**Proposal submission threshold:**

- Submit requires: 0.1% of circulating BRZA supply held by proposer, OR co-sponsorship by a recognized delegate
- Emergency proposals additionally require: 3+ Protocol steward co-signers

**Proposal types and thresholds:**

| Type | Approval required | Quorum | Min voting period |
|---|---|---|---|
| Core protocol change | 70% | 15% | 7 days |
| Treasury action | 60% | 10% | 3–7 days (admin configurable per community) |
| Constitutional / governance rule | 75% | 20% | 14 days |
| Working group / operational | 55% | 5% | 2 days |
| Emergency (pause, halt, critical patch) | 80% | 30% | 24 hours |

Community-track proposals use the community defaults (66% approval, 51% quorum, 7-day period) unless the community admin configures stricter values. Protocol-track proposals use the table above and are not configurable.

**Time-weighted voting multipliers (protocol track only):**

Holding BRZA for a longer consecutive period before a vote increases effective weight. Applies only to protocol-track votes, not community 1m1v votes.

| Continuous hold period | Voting weight multiplier |
|---|---|
| < 30 days | 1.0× (base) |
| ≥ 30 days | 1.2× |
| ≥ 90 days | 1.5× |
...
Snapshot timestamp: weight is calculated at the proposal creation block. Tokens transferred within 24 hours before snapshot are excluded.

**Delegation mechanics:**

- Minimum delegation lock: 7 days (delegator cannot un-delegate or re-delegate until lock expires)
- Max delegation concentration: no single delegate may hold more than 5% of total circulating protocol-track voting power
- Delegation APY incentive: 0.5–2% annualised on delegated BRZA (sourced from `communityRewards` pool — see `BRZA_EMISSION` in constants.ts)
- Delegate types: Technical (smart contracts, security), Treasury (financial policy), Grant (ecosystem allocation), Community (social/governance), Project (specific initiative)
- Delegation is protocol-track only — community 1m1v track does not support delegation
