 ...
 *
 * Wraps the on-chain Manager contract. Baraza has not deployed its own
 * Manager on Base yet — the addresses below are NOT_DEPLOYED placeholders.
 * Do not fill them in with any other protocol's addresses; when a Baraza
 * deployment lands, record it here and in contracts/evm/addresses/.
 *
...
// ─── Manager addresses (NOT_DEPLOYED placeholders) ──────────────────────────

const NOT_DEPLOYED = '0x0000000000000000000000000000000000000000' as const;

const MANAGER_ADDRESSES: Record<number, `0x${string}`> = {
  8453:  NOT_DEPLOYED, // Base mainnet — no Baraza Manager deployed yet
  84532: NOT_DEPLOYED, // Base Sepolia — no Baraza Manager deployed yet
};
