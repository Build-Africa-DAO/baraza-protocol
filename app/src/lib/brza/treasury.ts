...
export async function fetchTreasuryBalance(
  ...
  // Phase prices ship unset pending counsel review — value the balance at 0
  // rather than throwing so treasury reads stay functional.
  const priceUsd = isBrzaPriceConfigured(phase) ? getBrzaPriceUsd(phase) : 0;
  return {
    brza: formatBrza(raw),
