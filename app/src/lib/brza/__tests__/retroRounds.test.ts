...
describe('computeWeeklyRetroPool', () => {
  ...
  it('returns 0 while the monthly emission cap ships unset (counsel-gated)', () => {
    // BRZA_EMISSION.monthlyCapTokens is a 0 placeholder pending counsel
    // review, so every retro pool computes to 0 until it is configured.
    expect(computeWeeklyRetroPool(50, 10_000)).toEqual(0);
    expect(computeWeeklyRetroPool(500, 10_000)).toEqual(0);
    expect(computeWeeklyRetroPool(1000, 1000)).toEqual(0);
  });
});
