          ...
          <div className="mx-auto max-w-4xl">
            <div className="mb-6">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Order <span className="font-mono text-foreground">{orderId || "(none)"}</span> for{" "}
                {community?.name ?? "Community DAO"} is moving from M-Pesa confirmation to on-chain Membership Credential.
              </p>
            </div>

            {errorMessage && (
              <div className="mb-6 flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/8 p-4 text-sm">
            ...
            <div className="grid gap-6 lg:grid-cols-[0.62fr_0.38fr]">
              <div className="baraza-card p-5 md:p-6">
                <div className="space-y-5">
                  {stepStates.map((step) => (
                    <div key={step.code} className="flex gap-4 rounded-lg border border-border bg-background/45 p-4">
                      <div
                        className={
                          step.state === "done"
                      ...
                      <p className="self-center text-sm font-medium text-foreground">{step.label}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <aside className="space-y-5">
                <div className="baraza-card p-5">
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    <div className="flex justify-between gap-3 border-b border-border pb-3">
                      ...
                      <span className={statusIndex(status) >= statusIndex("PAYMENT_CONFIRMED") ? "text-confirmed" : "text-muted-foreground"}>
                    ...
                    <div className="flex justify-between gap-3">
                      ...
                      <span className={isComplete ? "text-confirmed" : "text-muted-foreground"}>
                ...
                {!publicKey && isComplete && (
                  <div className="rounded-lg border border-accent/25 bg-accent/8 p-5">
                    <p className="text-sm leading-6 text-muted-foreground">
