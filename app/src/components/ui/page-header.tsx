...
export function PageHeader({ title, subtitle, back, action, centered = false, className }: PageHeaderProps) {
  return (
    <header className={cn('mb-6', centered && 'text-center', className)}>
      {back ? (
        <div className={cn(centered && 'text-left')}>
          <Link
            to={back.to}
            className="mb-4 inline-flex min-h-12 items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"
          >
      ...
      <div className={cn('flex items-start justify-between gap-4', centered && 'justify-center')}>
        ...
        {action && !centered ? <div className="hidden shrink-0 md:block">{action}</div> : null}
      </div>
    </header>
  );
}
