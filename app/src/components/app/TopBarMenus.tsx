...
export function AccountMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <InitialsTile initials={initialsOf(account.displayName)} image={avatarUrl} size="sm" className="rounded-full" />
      ...
      {open ? (
        <div role="menu" aria-label="Account" className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-72 rounded-chrome bg-card p-2 shadow-deep">
          <div className="flex items-center gap-3 px-3 py-3">
            <InitialsTile
              initials={initialsOf(account.displayName)}
              image={avatarUrl}
              editable
              onImageChange={(newAvatar) => setAvatarUrl(newAvatar, account.getAccessToken)}
            />
...
export function LogoutMenu({ onBeforeLogout }: { onBeforeLogout?: () => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open ? (
        <div role="dialog" aria-label="Confirm log out" className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-72 rounded-chrome bg-card p-4 shadow-deep">
