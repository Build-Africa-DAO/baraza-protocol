...
**13 Sept 2026 — Frontend hardening before handoff (20 items), logged in `docs/FRONTEND_BACKEND_INTEGRATION.md` §14.** One API client with normalised errors and submit-once; Browse query fixed; votes and activity from the public tables; Baraza sign-in behind `VITE_AUTH_PROVIDER`; Google Identity Services; payout tracker on real statuses with a split plan; polling backoff; one-time fee; real Web Push; Account log-out confirm; Pages security headers; preview gallery out of the build; env example complete; hook rename and dead code removed; footer band and wordmark contrast fixed; axe clean in both themes bar the exempt logotype. 857 tests passing, typecheck and lint clean, build green.

**17 Sept 2026 — Production polish against Simon's spec (BRZ-FE-SPEC-2026-001), logged in `docs/FRONTEND_BACKEND_INTEGRATION.md` §15.** Twelve tickets: ten done, two blocked on backend data (EVM settlement state; STK expiry and Paybill). Lighthouse mobile on the production build went from 61 / 98 / 77 / 92 to 90 / 98 / 100 / 100 on Browse (83–91 performance across the four measured routes), by keeping wallet SDKs, framer-motion, zod and the Buffer polyfill off the first paint, self-hosting Geist, splitting the landing page around a static hero, converting the landing photos to right-sized WebP, and fixing the CSP so the inline theme script is allowed by hash. A build plugin now writes per-route modulepreload `Link` headers into `_headers` for Cloudflare's Early Hints. Touch targets are 48 px on phones app-wide. Pushback on the spec (countdown, Paybill, EVM badge, "WCAG 44 px", offline mutation queue, header errors, the unexplained 96.4%) is in §15.3.
