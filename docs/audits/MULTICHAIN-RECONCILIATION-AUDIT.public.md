...
| INTERNAL | Security | `blocked` | A live backend authorization control is reported disabled. A related authorization-helper fix exists only in a drafted migration, so repository evidence does not prove the live fix is applied. Details are retained in the private security record. |
...
| INTERNAL | Private backend migration | Payment authorization | Backend | `blocked` | A live authorization control is reported disabled. | Security-remediation PR after live posture is confirmed. |
| INTERNAL | Private backend migration | Admin authorization fix | Backend | `partial` | A corrected community-scoped authorization check is present in a drafted-only migration. | Confirm applied live state without exposing credentials. |
...
| INTERNAL | Backend row-level authorization | Launch blocker | A drafted migration does not enable the required policy. The directive states the live control is disabled; live state was not independently queried. |
| INTERNAL | Community-admin authorization fix | Present in source, live state unproven | A drafted migration binds the check to the current authenticated user, active membership, and privileged roles. |
| INTERNAL | Privileged database functions | Review required | One repository function uses elevated execution. Its detailed name and grant posture are retained in the private security record. |
...
| INTERNAL | Confidential branch history | The directive identifies confidential material in unpublished history. | No confidential content was inspected or reproduced. History purge remains maintainer work. |
...
| INTERNAL | Confirm whether the drafted authorization fix is applied and remediate the disabled live backend control | Repository source cannot prove live database state. |
