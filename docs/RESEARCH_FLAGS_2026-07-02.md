...
- **Raise:** a lean estimate of ~$130K–$310K, built bottom-up from a 6–12 month runway to mainnet + first cohort, not backed into from a round-number target. Grants-first. **Explicitly no centralized public IDO** — the model is communities/partners hold their own license (BlockCoop-style) and plug into Baraza as infrastructure, rather than BAD DAO taking on retail compliance itself.
...
1. **Allocation percentages** — the Notion table sums to 110% (a documented bug); the code fixed it with a different rebalance than a straight fix would produce.
2. **Referral mechanic** — Notion describes a 10-level percentage cascade; the live code has a completely different design (flat 500/250 BRZA payout per referral pair, gated by 90-day activity + 3 payments, monthly sub-cap, per-referrer cap, velocity-breach detection). This is a redesign, not a tuning difference.
3. **Baraza TV creator bucket** — 3% allocation exists in the live code, absent from the Notion table entirely.
